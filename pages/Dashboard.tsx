...
export const Dashboard: React.FC = () => {
  ...
  const getBarStyle = () => {
    ...
    const currentPercentageOfGoal = (todayCalories / dailyGoal) * 100;
    if (currentPercentageOfGoal <= 0) return { background: '#10b981' };

    const threshold = 80;
    const transitionPoint = (threshold / currentPercentageOfGoal) * 100;

    return {
      background: `linear-gradient(90deg, #10b981 0%, #10b981 ${transitionPoint}%, #f97316 ${transitionPoint}%, #f97316 100%)`,
      boxShadow: currentPercentageOfGoal >= 80 ? '0 0 12px rgba(249, 115, 22, 0.4)' : '0 0 12px rgba(16, 185, 129, 0.4)'
    };
  };
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className={`relative rounded-[32px] p-6 text-white overflow-hidden shadow-xl transition-all duration-500 ${todayCalories > dailyGoal ? 'bg-red-500 shadow-red-200 dark:shadow-red-900/40' : 'bg-royal-600 shadow-royal-200 dark:shadow-royal-900/40'}`}>
        ...
        <div className="relative z-10">
            ...
            <div className="w-full bg-black/20 rounded-full h-4 mb-3 overflow-hidden backdrop-blur-sm">
                <div 
                    className="h-full rounded-full transition-all duration-1000 ease-out will-change-[width]" 
                    style={{ 
                        width: `${progress}%`,
                        ...getBarStyle()
                    }}
      ...
      <button 
        ...
        className="fixed bottom-28 right-6 w-16 h-16 bg-royal-600 text-white rounded-[24px] shadow-[0_12px_24px_-6px_rgba(124,58,237,0.5)] flex items-center justify-center hover:scale-105 hover:bg-royal-700 active:scale-95 transition-all z-40"
      >
