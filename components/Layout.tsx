...
export const Layout: React.FC<LayoutProps> = ({ children, currentView, onNavigate }) => {
  return (
    <div className="min-h-screen bg-surface dark:bg-surface-dark flex flex-col max-w-md mx-auto relative shadow-2xl shadow-gray-200 dark:shadow-black/50 overflow-hidden border-x border-gray-100/50 dark:border-white/5 transition-colors duration-300">
      
      {/* Content Area */}
      <main className="flex-1 overflow-y-auto no-scrollbar pb-32 px-6 pt-10">
        {children}
      </main>

      {/* Bottom Navigation */}
      <div className="absolute bottom-8 left-6 right-6 z-50">
        <nav className="bg-white dark:bg-[#1a1c26] rounded-[32px] p-2 shadow-diffused-lg dark:shadow-diffused-dark border border-white dark:border-white/5 flex justify-between items-center">
            <button
