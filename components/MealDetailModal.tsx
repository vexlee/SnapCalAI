...
export const MealDetailModal: React.FC<MealDetailModalProps> = ({ entry, onClose, onDelete, onEdit }) => {
  ...
  const handleClearImage = async () => {
    if (confirm("Remove the photo to save storage space? Nutritional data will be kept.")) {
      ...
      try {
        const updatedEntry = { ...entry, imageUrl: undefined };
        await saveEntry(updatedEntry);
        // We trigger an edit/refresh by calling onEdit with the same but image-less entry 
        // Or simply close and let the parent refresh. Best is to close and refresh.
        onClose();
        // Since the parent usually re-fetches on success, we just close. 
        // In a real app, we might use a shared state or event bus.
